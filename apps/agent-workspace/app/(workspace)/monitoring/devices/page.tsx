    ...
    <div className="space-y-5 p-5">
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div className="space-y-1">
          ...
          <h1 className="font-display text-2xl font-bold tracking-tight">Device List</h1>
          <p className="text-sm text-muted-foreground">
            All {devices.data?.length ?? '—'} monitored devices across {sites.length - 1} site{sites.length > 2 ? 's' : ''}
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm"><RefreshCw className="h-3 w-3" />Refresh</Button>
          <Button size="sm"><Plus className="h-3 w-3" />Add device</Button>
        </div>
      </header>
